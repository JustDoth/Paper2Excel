...
脚本会先运行测试，再重新生成 `build/`、`dist/`、`release/Paper2Excel/` 和 `release/Paper2Excel-v0.1.0-windows.zip`。这些目录和 zip 是本机构建产物，不应提交到源码仓库；zip 应作为 GitHub Release 附件上传。

发布前请确认：

- `release/Paper2Excel/Paper2Excel.exe` 能正常启动。
- `release/Paper2Excel-v0.1.0-windows.zip` 解压后能直接运行。
...
2. 创建版本标签，例如 `v0.1.0`。
3. 上传脚本生成的 `release/Paper2Excel-v0.1.0-windows.zip` 作为 Release 附件。
